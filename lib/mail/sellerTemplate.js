const sellerTemplate = (sorurce) => {
    const id = sorurce?.orderId
    const date = sorurce?.orderedAt
  const name = sorurce?.customer?.fullName;
  const phone = sorurce?.customer?.mobileNum;
  const email = sorurce?.customer?.email;
  const totalAmount  = sorurce?.order?.priceDetails?.totalAmount
  const deliveryCharge = sorurce?.order?.priceDetails?.deliveryCharge
  const subTotal = sorurce?.order?.priceDetails?.subTotal
  const items = sorurce?.order?.products?.map((product) => {
    return `<tr>
